...
<form:form commandName="board" method="post" action="update.bd" enctype="multipart/form-data">
      	<input type="hidden" name="writer" value=${loginInfo.id }>
      	<input type="hidden" name="num" value=${board.num }>
      	<input type="hidden" name="pageNumber" value=${pageNumber }>
      	<input type="hidden" name="originalImg" value=${board.image }> <!-- 예전이미지담김 -->
      ...
      <label for="upload" class="form-label mt-4">그림파일</label>
      	...
      	<td>
      		<img height=100 width=100 
				src="<%=request.getContextPath()%>/resources/${board.image }">
				
    	  <input class="form-control" type="file" name="upload" id="formFile" value=${board.image }> <!-- 새로선택한이미지담김 -->
